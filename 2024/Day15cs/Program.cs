...
namespace Day15cs
{
	internal class Program
	{
		...
		{
			MovementSimulator simulator = new MovementSimulator(inputCol,false);
			simulator.Simulate(false);
			simulator.PrintLayout();
			return simulator.GetScore().ToString();
		}

		private static string GetResult2(List<string> inputCol)
		{
			MovementSimulator simulator = new MovementSimulator(inputCol, true);
