...
long long Simulation3(string tmp)
{
    long long step = 0;
    long long instLen = _instructions.size();
    while (true)
...
std::string MySolution3(std::vector<std::string>& vec)
{
    LoadData(vec);
    vector<string>startedA;
    for (pair<string, pair<string, string>> ps : map)
    ...
    vector<long long> sims;
    for (string s : startedA)
    {
        sims.push_back(Simulation3(s));
    }
    long long lcmVal = 1;

    for (long long l: sims)
    {
        lcmVal=lcm(lcmVal, l);
    }
    return std::to_string(lcmVal);
}
...
int main()
{
    ...
    std::string result = MySolution3(vec);
